...
class ConvAutoencoder(nn.Module):
    ...
    def forward(self, x):
        ...
        x = torch.sigmoid(
            self.t_conv4(x)
        )  # since after relu everything is positive, we need to use sigmoid to get the values between 0 and 1,
...
def create_image_features_df():
    ...
    for i in range(len(img_path_image)):
        df = df.append(
            {
                "path": img_path_image[i],
                "contrast": input_contrast[i].item(),
                "mean_r": input_mean_rgb[i][0].item(),
                "mean_g": input_mean_rgb[i][1].item(),
                "mean_b": input_mean_rgb[i][2].item(),
            },
            ignore_index=True,
        )
...
def split_images_to_photographers():
    ...
    scatter_clustering_with_gt_labels_in_2d(
        df[["contrast", "mean_r", "mean_g", "mean_b"]], clusterer.labels_
    )
    scatter_clustering_with_gt_labels_in_3d(
        df[["contrast", "mean_r", "mean_g", "mean_b"]], clusterer.labels_
    )
...
if __name__ == "__main__":
    ...
    for epoch in range(1 + previous_epoch_idx, n_epochs + 1 + previous_epoch_idx):
        ...
        for batch_idx, batch in tqdm(
            ...
            input_contrast[torch.isinf(input_contrast)] = input_contrast[
                ~torch.isinf(input_contrast)
            ].mean()

            contrast_loss = (
                loss_func(
                    input_contrast.type(torch.float), output_contrast.type(torch.float)
                )
                / 100000
            )
            ...
            if save_embedding:
                ...
                embedding_file_path = (
                    f"embeddings/embedding_epoch_{epoch}_{flags.timestamp}.csv"
                )
            ...
            if batch_idx % 20 == 0:
                ...
                Path(f"./model_weights/{flags.timestamp}").mkdir(
