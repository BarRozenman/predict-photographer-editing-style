...
class CustomDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        items_dict = {
            ...
            "images_editing_features": torch.tensor(
                self.images_editing_features[idx], dtype=torch.float32
            ),
...
class MixedNetwork(nn.Module):
    def __init__(self, norm_params=None, train=True):
        ...
        self.combined_features = nn.Sequential(
            nn.Linear(13, 8), nn.ReLU(), nn.Linear(8, 8), nn.ReLU(), nn.Linear(8, 4)
        )

    def norm_test_input(self, img_features, user_features):

        img_features = (img_features - self.norm_params["images_embeddings_min"]) / (
            self.norm_params["images_embeddings_max"]
            - self.norm_params["images_embeddings_min"]
        )
    ...
    def forward(self, img_features, user_features):
        if self.train:
            ...
            img_features, user_features = self.norm_test_input(
                img_features, user_features
            )
...
def plot_user_temporal_embedding(users_embed_temporal, loss_for_plot, sink_path):
    ...
    last_users_embed_temporal = pd.DataFrame(
        users_embed_temporal[-1, :, :], columns=["x", "y", "z"]
    )
    ...
    for i in range(users_embed_temporal.shape[0]):
        ...
        dim_2_users = pd.DataFrame(
            dim_reduction_func.transform(curr_users_embed), columns=["x", "y"]
        )
        ax = sns.scatterplot(
            data=dim_2_users, x="x", y="y", hue=dim_2_users.index.values, palette="deep"
        )
        ax.get_legend().remove()
        ax.set_xlim(-1.2, 1.2)
        ax.set_ylim(-1.2, 1.2)
        plt.title(
            f"Users embedding at step {str(i).zfill(4)}\n Loss: {round(loss_for_plot[i], 5):8.3f}"
        )
...
def crawl_images_from_users_embedding_frames(curr_time):
    ...
    generate_video_from_frames_paths(
        frames, f"visuals/user_embed_temporal_{flags.timestamp}.mp4", fps=8
    )
...
if __name__ == "__main__":
    ...
    common_paths = np.intersect1d(
        images_editing_features["path"].values, im_emb.index.values
    )
    ...
    norm_params = {
        "images_editing_features_min": v.min(0),
        "images_editing_features_max": v.max(0),
    }
    ...
    loss_for_plot = [1]
    for epoch in tqdm(
        range(1 + previous_epoch_idx, n_epochs + 1 + previous_epoch_idx), total=n_epochs
    ):
        for batch_idx, data in enumerate(train_loader):
            global_step = (
                previous_global_step + batch_idx + (epoch - 1) * len(train_loader)
            )
            ...
            if global_step % 200 == 0:
                ...
                with torch.no_grad():
                    for data in test_loader:
                        image_embed = data["image_embed"].to(device)
                        images_editing_features = data["images_editing_features"].to(
                            device
                        )
                        label = data["images_editing_features"].to(device)
                        user_embed_idx = data["label"]
                        ...
                        user_embed = torch.tensor(
                            user_embed, requires_grad=True, device=device
                        )
                        pred = model(image_embed, user_embed)
                ...
                users_embed_temporal = torch.cat(
                    (users_embed_temporal, users_embed.unsqueeze(0)), dim=0
                )
                ...
                torch.save(
                    model,
                    f"{dir}/edit_predictor_{flags.timestamp}_{epoch}_{global_step}.pt",
                )
        scheduler.step()
        print("Epoch: {} \tTraining Loss: {:.6f}".format(epoch, loss.item()))
    torch.save(
        users_embed_temporal[-1, ...],
        f"user_embedding/users_embed_temporal_{flags.timestamp}.pt",
    )
    plot_user_temporal_embedding(
        users_embed_temporal,
        loss_for_plot,
